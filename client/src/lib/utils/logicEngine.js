...
/**
 * Runtime invariants:
 * 1) Navigation logic is evaluated only from explicit navigation intent (Next/Submit).
 * 2) Visibility determines validation eligibility (hidden questions are non-applicable).
 * 3) Preview and live runtime must share this exact logic behavior.
 * 4) Navigation does not change condition truth; only answer changes can change visibility outcomes.
 */
 ...
 * @param {string} mode - Mode: 'live' (default) or 'preview'
...
export function getVisibleQuestionIds(
  ...
  // Preview and live share identical visibility evaluation semantics.
...
export function getAnswerableQuestionIds(
  ...
  for (const question of questions || []) {
    ...
    const sectionId =
      sectionByQuestionId.get(question.id) || question.sectionId;
    if (
      !sectionId ||
...
/**
 * Evaluate highest-priority navigation action for current runtime state
 *
 * Nested navigation semantics:
 * - Rules are evaluated in deterministic order (priority desc, then stable key order).
 * - First matching rule wins; subsequent rules are ignored for this step.
 * - If no rule matches, caller falls back to normal sequential flow.
 * - Forward-only question jump guard is available via `enforceForwardQuestionJump`.
 * - Visibility/display logic is evaluated separately and does not by itself trigger navigation.
 *
 * @param {string} currentSectionId - Current section ID
 * @param {Array} sections - All sections in survey (ordered)
 * @param {Array} questions - All survey questions
 * @param {Array} navigationRules - Navigation rules to evaluate
 * @param {Map|Object} answers - Current response answers
 * @param {Set} visibleSectionIds - Set of currently visible section IDs
 * @param {Set} visibleQuestionIds - Set of currently visible question IDs
 * @param {string|null} triggeredQuestionId - Optional question ID that changed
 * @param {Set|Array|null} allowedQuestionIds - Optional scope of questions allowed to drive navigation in this step
 * @param {boolean} enforceForwardQuestionJump - When true, block backward/self question jumps
 * @returns {Object|null} Navigation decision object or null
 */
export function evaluateNavigationAction(
  currentSectionId,
  sections,
  questions = [],
  navigationRules = [],
  ...
  visibleSectionIds,
  visibleQuestionIds,
  triggeredQuestionId = null,
  allowedQuestionIds = null,
  enforceForwardQuestionJump = false
) {
  ...
  for (const section of sections || []) {
    for (const questionId of section?.questionIds || []) {
      if (questionId && !sectionByQuestionId.has(questionId)) {
        sectionByQuestionId.set(questionId, section.id);
      }
    }
  }

  const allowedQuestionSet = Array.isArray(allowedQuestionIds)
    ? new Set(allowedQuestionIds)
    : allowedQuestionIds instanceof Set
      ? allowedQuestionIds
      : null;

  const sortedRules = normalizedNavigationRules
    .filter((rule) => {
      const sourceQuestionId = rule.when?.questionId;
      const sourceSectionId = sourceQuestionId
        ? sectionByQuestionId.get(sourceQuestionId)
        : null;
      const fromSectionMatches =
        rule.fromSectionId === currentSectionId ||
        rule.fromSectionId === null ||
        (!!currentSectionId && sourceSectionId === currentSectionId);
      if (!fromSectionMatches) return false;
      if (
        triggeredQuestionId &&
        rule.when?.questionId !== triggeredQuestionId
      ) {
        return false;
      }
      if (
        ...
        allowedQuestionSet &&
        rule.when?.questionId &&
        !allowedQuestionSet.has(rule.when.questionId)
      ) {
        return false;
      }
      return true;
  ...
  for (const rule of sortedRules) {
    ...
    if (actionType === "terminate") {
      return { type: "terminate", ruleId: rule.id };
    }

    if (actionType === "jump") {
      const targetSectionId = rule.action?.targetSectionId || rule.targetSectionId;
      if (
        targetSectionId &&
        (!visibleSectionIds || visibleSectionIds.has(targetSectionId))
      ) {
        return {
          type: "jump_section",
          targetSectionId,
    ...
    if (actionType === "jump_to_question") {
      const targetQuestionId = rule.action?.targetQuestionId || rule.targetQuestionId;
      if (!targetQuestionId) continue;
      if (visibleQuestionIds && !visibleQuestionIds.has(targetQuestionId)) {
        continue;
      }

      if (enforceForwardQuestionJump && rule.when?.questionId) {
        const sourceIndex = questions.findIndex(
          (question) => question.id === rule.when.questionId
        );
      ...
      const targetSectionId = findSectionIdForQuestion(
        targetQuestionId,
        sections,
        questions
      );
      if (
        targetSectionId &&
        visibleSectionIds &&
        !visibleSectionIds.has(targetSectionId)
      ) {
        continue;
      }
      ...
      return {
        ...
        targetQuestionId,
        targetSectionId,
        sourceQuestionId: rule.when?.questionId || null,
        ruleId: rule.id,
      };
    }
    ...
    if (actionType === "skip") {
      return {
        type: "skip",
        sourceQuestionId: rule.when?.questionId || null,
        skipCount: Number.isFinite(rule.action?.skipCount)
 ...
 * This handles intra-section question hiding when jump conditions are met
 *
...
export function computeVisibleQuestionsInSection(
  ...
  // If no navigation rules, return all questions
  if (!Array.isArray(navigationRules) || navigationRules.length === 0) {
    return sectionQuestions;
  }

  // Use canonical incoming section order. This order is already curated by
  // runtime section/question assembly and should not be reshuffled here.
  ...
  const getTargetQuestionId = (rule) =>
    rule?.action?.targetQuestionId || rule?.targetQuestionId || null;
  const getSectionScopedActionType = (rule) => {
    const actionType = getRuleActionType(rule?.action);
    if (
      (actionType === "jump" || !actionType) &&
      typeof getTargetQuestionId(rule) === "string"
    ) {
      return "jump_to_question";
    }
    return actionType;
  ...
  // Get relevant jump/skip rules for this section
  const relevantRules = localizedNavigationRules
    .filter((rule) => {
      ...
      const actionType = getSectionScopedActionType(rule);
  ...
  for (const rule of relevantRules) {
    ...
    const targetQuestionId = getTargetQuestionId(rule);
  ...
  for (const sourceQuestionId of orderedSources) {
    ...
    if (!hasAnswer(sourceQuestionId)) {
      // Source question not answered yet:
      // keep everything after the source hidden until respondent picks a branch.
      for (
    ...
    if (matchedRule) {
      // Hide questions skipped by the active jump branch.
      for (
        let questionIndex = sourceIndex + 1;
      ...
      // Keep branch window isolated: only the selected branch target remains
      // visible inside the source branch window.
      for (
      ...
      // Hide sibling jump targets that were not selected for this source.
    ...
    // No jump condition matched after source answered:
    // keep explicit jump targets hidden on the continue path.
    if (
  ...
  // Exit-branch handling:
  // If source answer matches a jump that exits this section, hide all following
  // local questions so branch UI does not show unrelated siblings.
    ...
    .filter((rule) => {
      ...
      if (actionType === "jump_to_question") {
        ...
        const isExternalQuestionJump =
          ...
          !Number.isFinite(targetIndex) &&
          globalQuestionIdSet.has(targetQuestionId);
        // Cross-section/unmapped target for current section.
        return isExternalQuestionJump;
      }
...
/**
 * Compute visible questions for question-flow mode based on answered jump branches.
 * - Keeps default visibility until a source answer exists.
 * - When a jump rule matches, hides skipped intermediate questions.
 * - When multiple jump targets exist for the same source, hides non-selected targets.
 *
 * @param {Array} orderedQuestions - Ordered visible questions
 * @param {Array} navigationRules - Navigation rules to evaluate
 * @param {Map|Object} answers - Current response answers
 * @returns {Array} Branch-pruned ordered questions
 */
export function computeVisibleQuestionsForQuestionFlow(
  orderedQuestions = [],
  navigationRules = [],
  answers = {}
) {
  ...
  for (const rule of normalizedNavigationRules) {
    const actionType = getRuleActionType(rule.action);
    if (actionType !== "jump_to_question") continue;
  ...
  const orderedSources = [...rulesBySource.keys()].sort(
    (a, b) => (indexByQuestionId.get(a) || 0) - (indexByQuestionId.get(b) || 0)
  );

  for (const sourceQuestionId of orderedSources) {
    if (!hasAnswer(sourceQuestionId) || hiddenQuestionIds.has(sourceQuestionId)) {
      continue;
    }

    const sourceRules = rulesBySource
      .get(sourceQuestionId)
      .sort(compareRuleEntries);
    const matchedRule = sourceRules.find((entry) =>
      evaluateCondition(entry.rule.when, answers)
    ...
    // Hide skipped intermediate questions for this jump.
    for (
      let questionIndex = matchedRule.sourceIndex + 1;
      questionIndex < matchedRule.targetIndex;
      questionIndex += 1
    ) {
      const questionToHide = orderedQuestions[questionIndex];
    ...
    // Hide non-selected sibling jump targets from the same source.
    for (const sourceRule of sourceRules) {
      if (sourceRule.targetQuestionId !== matchedRule.targetQuestionId) {
        hiddenQuestionIds.add(sourceRule.targetQuestionId);
      }
    }
  }

  if (hiddenQuestionIds.size === 0) return orderedQuestions;
  return orderedQuestions.filter((question) => !hiddenQuestionIds.has(question.id));
}

/**
 * Get orphaned answer keys that should be removed when questions become hidden
 *
 * @param {Array} previousVisibleQuestions - Previously visible questions
 * @param {Array} currentVisibleQuestions - Currently visible questions
 * @param {Map|Object} answers - Current response answers
 * @returns {Array} Array of answer keys to remove
 */
export function getOrphanedAnswerKeys(
  previousVisibleQuestions = [],
  currentVisibleQuestions = [],
  answers = {}
) {
  const previousVisibleIds = new Set(
    previousVisibleQuestions.map((q) => q.id)
  );
  const currentVisibleIds = new Set(
    currentVisibleQuestions.map((q) => q.id)
  );

  const orphanedKeys = [];
  const answersObj = answers instanceof Map ? Object.fromEntries(answers) : answers;

  // Find questions that are no longer visible
  for (const questionId of previousVisibleIds) {
    if (!currentVisibleIds.has(questionId)) {
      // Check if this question has answers that need to be removed
      if (answersObj[questionId] !== undefined) {
        orphanedKeys.push(questionId);
      }
      // Also check for "other" text fields
      if (answersObj[`${questionId}_other_text`] !== undefined) {
        orphanedKeys.push(`${questionId}_other_text`);
      }
    }
  }
...
function findSectionIdForQuestion(questionId, sections = [], questions = []) {
  for (const section of sections || []) {
